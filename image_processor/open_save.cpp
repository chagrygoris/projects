...
void CreateInfoHeader(unsigned char *info_header, const int file_size, int width, int height, int x_pixels,
    ...
    info_header[1] = 0;
    info_header[2] = 0;
    info_header[3] = 0;  // NOLINT

    info_header[4] = width;        // NOLINT
...
void SaveFile(const char *path, Image &image) {
    ...
    unsigned char padding[3] = {0, 0, 0};                            // NOLINT
    const int count_paddings = ((4 - (image.Width() * 3) % 4) % 4);  // NOLINT
    const int file_size =
    ...
    for (int y = 0; y < image.Height(); ++y) {
        for (int x = 0; x < image.Width(); ++x) {
            ...
            unsigned char pixel_r = static_cast<unsigned char>(std::round(temp.r_ * 255.0f));  // NOLINT
            unsigned char pixel_g = static_cast<unsigned char>(std::round(temp.g_ * 255.0f));  // NOLINT
            unsigned char pixel_b = static_cast<unsigned char>(std::round(temp.b_ * 255.0f));  // NOLINT

            unsigned char pixel_color[3] = {pixel_b, pixel_g, pixel_r};  // NOLINT
            f.write(reinterpret_cast<char *>(pixel_color), 3);           // NOLINT
        }
...
void Image::Read(const char *path) {
    ...
    const int count_paddings = ((4 - (width_ * 3) % 4) % 4);  // NOLINT
    for (int y = 0; y < height_; ++y) {
        for (int x = 0; x < width_; ++x) {
            unsigned char pixel_color[3];  // NOLINT
